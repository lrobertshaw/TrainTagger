...
def _define_target(data):
    ...
    data['target_pt'] = np.log2( np.clip( data["jet_genmatch_pt"], 1, 2048 ) )    #np.clip(pt_ratio, 0.3, 3)
    data['target_pt_phys'] = np.clip( data["jet_genmatch_pt"], 1, 2048 )

    data["target_mass"] = np.log2( np.clip( data["jet_genmatch_mass"], 1, 256 ) )    #np.clip(mass_ratio, 0.3, 3)
    data['target_mass_phys'] = np.clip( data["jet_genmatch_mass"], 1, 256)
